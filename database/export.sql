...
INSERT INTO `block_admin` VALUES (101,'shiva',1),(102,'rajaa',2);
...
CREATE TABLE `identity` (
  ...
  KEY `fk_t` (`tenant_id`),
  CONSTRAINT `fk_t` FOREIGN KEY (tenant_id) REFERENCES tenant(tenant_id) ON DELETE CASCADE,
...
CREATE TABLE `tenant` (
  ...
  `stat` varchar(10) DEFAULT 'not paid',
  `room_no` int DEFAULT NULL,
  `age` int DEFAULT NULL,
...
INSERT INTO `tenant` VALUES (601,'nithin','01-apr-02','paid',11,19),(602,'rohith','23-aug-02','not paid',12,23),(603,'mothi','12-jun-02','paid',13,41),(604,'abu danish','23-apr-02','not paid',21,35),(605,'Hari','30-sep-02','not paid',31,56);
...
DROP PROCEDURE IF EXISTS GetActiveTenants;
DELIMITER //

DELIMITER //

CREATE PROCEDURE GetActiveTenants()
BEGIN
    SELECT 
        t.tenant_id AS `Tenant no`,
        r.room_no AS `Room_no`,
        t.name AS `Name`,
        TIMESTAMPDIFF(YEAR, STR_TO_DATE(t.dob, '%Y-%m-%d'), CURDATE()) AS `Age`,
        t.dob AS `DOB`,
        t.stat AS `Stat`,
        r.type AS `Room_type`,        -- Added Room Type
        b.block_name AS `Block`      -- Added Block Name
    FROM tenant t
    JOIN room r ON t.room_no = r.room_no
    JOIN block b ON r.block_no = b.block_no
    WHERE t.stat = 'not paid';
END //

DELIMITER ;


-- SELECT * from complaint_log;
--
