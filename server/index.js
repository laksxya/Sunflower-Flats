...
app.post("/auth", (req, res) => {
  ...
  const result =db.authoriseuser(username,password,(err,result)=>{
...
app.get('/unpaidtenantdetails', (req, res) => {
    db.getunpaidtenants('GetActiveTenants', (err, result) => {
        if (err) {
            res.status(500).send({ error: "Error executing stored procedure" });
        } else {
            res.send(result[0]); // Send only the first result set
        }
    });
});
...
app.post('/deleteidentity', function(req, res) {
    const ownerId = req.body.ownerId;
    db.deleteowner(ownerId, (err, result) => {
        if (err) {
            console.log(err);
            res.status(500).send({ error: 'Error deleting owner' });
            return;
        }
...
// Route to get details of unpaid tenants
